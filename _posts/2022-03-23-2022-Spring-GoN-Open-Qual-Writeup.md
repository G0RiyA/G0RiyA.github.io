...
이때, 위 과정을 정확하게 수행하기 위해서 동적디버깅을 몇 시간에 걸쳐서 오프셋을 모두 뽑았다 ㅠ
...
\+ 대회가 끝난 후에 출제자 writeup을 봤을 땐 UTF-8 문제가 없어보여서 이유가 궁금하다.
...
하지만 그 알 수 없는 연산은 대충 봤을 때, 지옥 불구덩이의 용암 같은 열정도 당장 베스킨라빈스 아이스크림으로 만들 정도로 분석하고 싶지 않게 생겨 먹었다.

해당 연산을 수행하는 함수에 2 byte씩 대입하는 코드를 작성하여 플래그를 얻기로 하였고\
그래서 앵거를 사용하면 좋았겠지만, 사용법을 공부한 적이 없어서 gdb script를 통해 스크립트를 작성하였다.
...
fgets를 통해 48인 입력을 받고, 입력을 길이가 16인 블록으로 쪼개서 연산한다.

연산은 대충 x라고 정의한 `[0x21, 0xE5, 0x88, 0xAC, 0xBB, 0xB0, 0x97, 0xEA, 0x16, 0x42, 0x03, 0x0B, 0x9B, 0xD2, 0x5C, 0x6C]` 값과 함께 진행된다.
...
여러 변수를 고려하고도 다른 문자 대비 차가 큰 문자 하나가 존재한다.
